...
class phpMyImporter {
	...
	*/
	function phpMyImporter($db=null, $connection=null, $filepath='dump.sql', $compress=false) {
		...
		return $this->setDatabase($connection, $db);
	}
	...
	function importSql($file) {
		...
		foreach ($lines as $line) {
			...
			if ( substr(trim($line), strlen(trim($line))-1) == ";" ) {
				$query = @mysqli_query($this->connection, $importSql);
	...
	*/
	function doImport() {		
		if ( !$this->setDatabase($this->connection, $this->database) )
			return false;
