...
;(function ( $, window, document, undefined ) {
	...
	VariationForm.prototype.onFoundVariation = function( event, variation ) {
		...
		if( typeof price != 'undefined' && price.trim().length > 0 ) {
			var html_content = $( '.content_product_detail > div[itemprop="offers"]' ).html();
			if( !html_content.match( /variable-price/ ) ){
				$( '.content_product_detail > div[itemprop="offers"]' ).append( '<span class="variable-price"></span>' );
			}
			if ( ! variation.variation_is_visible ) {
				...
				$( 'div[itemprop="offers"] p.price' ).hide();				
				$( '.variable-price' ).show().html( price );
			}
	...
	$.fn.wc_variations_image_update = function( variation ) {
		var $form             = this,
			...
			$gallery_img_wrap = $product_img_wrap.hasClass( 'item-video' ) ? $product_gallery.find( '.product-responsive-thumbnail .item-thumbnail-product.slick-active:eq(1)' ) : $product_gallery.find( '.product-responsive-thumbnail .item-thumbnail-product.slick-active:eq(0)' ) ,
			$gallery_img			= $gallery_img_wrap.find( 'img' ),
		...
		if ( variation && variation.image && variation.image.src && variation.image.src.length > 1 ) {
			...
			$product_gallery.find( '.product-responsive-thumbnail .item-thumbnail-product.slick-active' ).removeClass( 'slick-current' );
			if( $( 'body' ).hasClass( 'product-video' ) ){
				$gallery_img.trigger( 'click' );
			}
			$gallery_img_wrap.addClass( 'slick-current' );			
		} else {
	...
	$('[data-toogle="tooltip"]').on('mouseenter', function(){
		var variation_content = $(this).html();
		var data_img = $(this).data( 'img' ), data_width = $(this).data( 'width' ), data_height = $(this).data( 'height' ), offset_left = -( data_width - $(this).outerWidth() )/2;
		var content_html = '<img src="'+ data_img +'"/>';
		if( !variation_content.match( /variation-tooltip/ ) ){
			$(this).append('<div class="variation-tooltip"></div>');
		}
		var offset = $(this).offset(),  offset_top = 0;
		if( offset.top < data_height ){
			$( '.variation-tooltip' ).css( 'top', '110%' );
		}
		$( '.variation-tooltip' ).css({ 'width': data_width, 'height': data_height, 'left': offset_left });
		$( '.variation-tooltip' ).html(content_html);
	
	}).on( 'mouseleave', function(){
		$( '.variation-tooltip' ).html('');
	});
