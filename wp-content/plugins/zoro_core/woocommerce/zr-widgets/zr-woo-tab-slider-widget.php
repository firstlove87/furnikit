 ...
 */
if ( !class_exists('zr_woo_tab_slider_widget') ) {
	class zr_woo_tab_slider_widget extends WP_Widget {
		...
		function __construct() {
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'zr_woo_tab_slider', 'description' => __('Zr Woo Tab Slider', 'zr_core') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'zr_woo_tab_slider' );

			/* Create the widget. */
			parent::__construct( 'zr_woo_tab_slider', __('Zr Woo Tab Slider widget', 'zr_core'), $widget_ops, $control_ops );
			...
			if( version_compare( WC()->version, '2.4', '>=' ) ){
					add_action( 'wc_ajax_zr_tab_category', array( $this, 'zr_tab_category_callback' ) );
			}
			else {
				add_action( 'wp_ajax_zr_tab_category', array( $this, 'zr_tab_category_callback') );
				add_action( 'wp_ajax_nopriv_zr_tab_category', array( $this, 'zr_tab_category_callback') );
			}
		}
		...
		**/
		function SC_integrateWithVC(){
			...
			vc_map( array(
				"name" => __( "ZR Woocommerce Tab Slider", 'zr_core' ),
				"base" => "woo_tab_slider",
				"icon" => "icon-wpb-ytc",
				"class" => "",
				...
				array(
				 ...
				 array(
					'type' => 'attach_images',
					'heading' => __( 'Banner Images', 'zr_core' ),
					'param_name' => 'images',
		...
		**/
		function SC_WooTab( $atts, $content = null ){
			...
			if( $layout == 'default' ){
				...
				include( zr_override_check( 'zr-woo-tab-slider', 'theme1' ) );
			}
			elseif( $layout == 'layout2' ){
				include( zr_override_check( 'zr-woo-tab-slider', 'theme2' ) );
			}
			elseif( $layout == 'layout3' ){
				include( zr_override_check( 'zr-woo-tab-slider', 'theme-mobile' ) );
			}
		...
		function zr_tab_category_callback(){
