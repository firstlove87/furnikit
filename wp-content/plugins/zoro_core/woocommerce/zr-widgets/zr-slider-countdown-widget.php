<?php
/**
	* ZR Woocommerce Countdown Slider
	* Register Widget Woocommerce Countdown Slider
	* @author 		flytheme
	* @version     1.0.0
**/
if ( !class_exists('zr_woo_slider_countdown_widget') ) {
	class zr_woo_slider_countdown_widget extends WP_Widget { 
		...
		function __construct(){
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'zr_woo_slider_countdown_widget', 'description' => __('Zr WooCommerce Countdown', 'zr_core') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'zr_woo_slider_countdown_widget' );

			/* Create the widget. */
			parent::__construct( 'zr_woo_slider_countdown_widget', __('Zr WooCommerce Countdown Widget', 'zr_core'), $widget_ops, $control_ops );
		...
		**/
		function WS_Countdown_integrateWithVC(){
			...
			vc_map( array(
			  "name" => __( "ZR Woocommerce Countdown", 'zr_core' ),
				...
				array(
				 ...
				 array(
				  ...
				  array(
					...
					"param_name" => "layout",
					"admin_label" => true,
					"value" => array( 'Layout Default' => 'default', 'Layout Countdown 1' => 'layout1' ),
		...
		**/
		function WS_Shortcode_Countdown( $atts, $content = null ){
			...
			ob_start();		
			if( $layout == 'default' ){
				include( zr_override_check( 'zr-countdown-slider', 'countdown' ) );				
			}
			elseif( $layout == 'layout1' ){
				include( zr_override_check( 'zr-countdown-slider', 'countdown1' ) );			
			}
		...
		**/
		...
		public function widget( $args, $instance ) {
			...
			if ( $tpl = zr_override_check( 'zr-countdown-slider', $instance['widget_template'] ) ){ 			
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo $this->get_field_id('description1'); ?>"><?php _e('Description', 'zr_core')?></label>
				<br />
			...
			<p>
				...
				<select class="widefat"
					...
					<?php } ?>>
						<?php _e('Layout Countdown 2', 'zr_core')?>
					</option>
				</select>
			</p>  
		<?php
