...
/**
	...
	* @version     1.0.0
**/
if ( !class_exists('zr_related_upsell_widget') ) {
	class zr_related_upsell_widget extends WP_Widget {
		/**
		 * Widget setup.
		 */
		function __construct(){
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'zr_related_upsell_widget', 'description' => __('Zr Woocommerce Related & Upsell', 'zr_core') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'zr_related_upsell_widget' );

			/* Create the widget. */
			parent::__construct( 'zr_related_upsell_widget', __('Zr Woocommerce Related & Upsell widget', 'zr_core'), $widget_ops, $control_ops );
		...
		protected function getTemplatePath($tpl='default', $type=''){
			$file = '/'.$tpl.$type.'.php';
			$dir =	plugin_dir_path(dirname(__FILE__)).'/themes/zr-reup-slider';
		...
		public function form( $instance ) {
			...
			<p>
				...
				<select class="widefat"
					...
					<?php } ?>>
						<?php _e('Related Slider', 'zr_core')?>		
					</option>
					<option value="related-mobile" <?php if ($widget_template=='related-mobile'){?> selected="selected"
					<?php } ?>>
						<?php _e('Related Mobile', 'zr_core')?>		
					</option>					
					<option value="upsell" <?php if ($widget_template=='upsell'){?> selected="selected"
					<?php } ?>>
						<?php _e('Upsell Slider', 'zr_core')?>
					</option>
					<option value="tab_reup" <?php if ($widget_template=='tab_reup'){?> selected="selected"
					<?php } ?>>
						<?php _e('Tab Related & Upsell Slider', 'zr_core')?>
					</option>					
				</select>
			</p>  
		<?php
