...
if( !class_exists('zr_resp_slider') ) :
	add_action( 'widgets_init', 'zr_resp_register' );
	function zr_resp_register(){
			register_widget( 'zr_resp_slider' );
	}
	 
	class zr_resp_slider extends WP_Widget {
		...
		function __construct() {
			...
			/* Widget settings. */
			$widget_ops = array( 'classname' => 'zr_resp_slider', 'description' => __('Zr Responsive Slider Widget', 'zr_core') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'zr_resp_slider' );

			/* Create the widget. */
			parent::__construct( 'zr_resp_slider', __('Zr Responsive Slider Widget', 'zr_core'), $widget_ops, $control_ops );
		...
		**/
		function RP_integrateWithVC(){
			...
			vc_map( array(
				"name" => __( "ZR Responsive Post Slider", 'zr_core' ),
				"base" => "respost_slide",
				"icon" => "icon-wpb-ytc",
				"class" => "",
				...
				'heading'       => __('Category filter:', 'zr_core'),
				'description'   => '',
				'holder'        => 'div',
				'class'         => ''
				 ),
		...
		**/
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo $this->get_field_id('title2'); ?>"><?php _e('Title', 'zr_core')?></label>
				<br />
