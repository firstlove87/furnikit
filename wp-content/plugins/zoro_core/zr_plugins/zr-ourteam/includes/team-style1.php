<?php 
	...
	$id = $this -> number;
?>
<div id="zr_ourteam_<?php echo esc_attr( $id ) ?>" class="responsive-slider zr-ourteam-slider style1 loading clearfix" data-lg="<?php echo esc_attr( $columns ); ?>" data-md="<?php echo esc_attr( $columns1 ); ?>" data-sm="<?php echo esc_attr( $columns2 ); ?>" data-xs="<?php echo esc_attr( $columns3 ); ?>" data-mobile="<?php echo esc_attr( $columns4 ); ?>" data-speed="<?php echo esc_attr( $speed ); ?>" data-scroll="<?php echo esc_attr( $scroll ); ?>" data-interval="<?php echo esc_attr( $interval ); ?>" data-rtl="<?php echo ( is_rtl() || $zr_direction == 'rtl' )? 'true' : 'false';?>" data-autoplay="<?php echo esc_attr( $autoplay ); ?>">
	<div class="box-title">
						...
						<div class="item-desc">
							<?php 
								if ( preg_match('/<!--more(.*?)?-->/', $post->post_content, $matches) ) {
									...
									$content = self::zr_trim_words($post->post_content, $length, ' ');
								}
