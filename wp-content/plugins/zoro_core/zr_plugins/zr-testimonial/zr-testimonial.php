<?php
/**
 * Name: ZR Testimonial Slider Widget
 * Description: A widget that serves as an slider for developing more advanced widgets.
 */

class zr_testimonial_slider_widget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zr_testimonial_slider', 'description' => __('Zr Testimonial Slider', 'zr_core') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'zr_testimonial_slider' );

		/* Create the widget. */
		parent::__construct( 'zr_testimonial_slider', __('Zr Testimonial Slider widget', 'zr_core'), $widget_ops, $control_ops );
	...
	function testimonial_init(){
		add_meta_box( __( 'Testimonial Options', 'zr_core' ), __( 'Testimonial Options', 'zr_core' ), array( $this, 'testimonial_detail' ), 'testimonial', 'normal', 'low' );
	}
	
	function testimonial_detail(){
		global $post;
		// Add an nonce field so we can check for it later.
		wp_nonce_field( 'testimonial_save_meta', 'zr_testimonial_plugin_nonce' );
	...
	?>	
		<p><label><b><?php esc_html_e('Author Name', 'zr_core'); ?>:</b></label><br/>
			<input type ="text" name = "au_name" value ="<?php echo esc_attr( $au_name );?>" size="70%" /></p>
		<p><label><b><?php esc_html_e('Author URL', 'zr_core'); ?>:</b></label><br/>
			<input type ="text" name = "au_url" value ="<?php echo esc_attr( $au_url );?>" size="70%" /></p>
		<p><label><b><?php esc_html_e('Author Infomation', 'zr_core'); ?>:</b></label><br/>
			<input type ="text" name = "au_info" value ="<?php echo esc_attr( $au_info );?>" size="70%" /></p>
	<?php 
	}
	function testimonial_save_meta(){
		global $post;
		if ( ! isset( $_POST['zr_testimonial_plugin_nonce'] ) ) {
			return;
		}
		if ( ! wp_verify_nonce( $_POST['zr_testimonial_plugin_nonce'], 'testimonial_save_meta' ) ) {
			return;
		}
	...
	**/
	function TT_integrateWithVC(){
		vc_map( array(
			'name' =>  __( 'Zr Testimonial Slider', 'zr_core' ),
			'base' => 'testimonial',
			"icon" => "icon-wpb-ytc",
			...
			'params' => array(
				array(
				 ...
				 array(
					...
					"holder" => "div",
					"class" => "",
					"heading" => __( "Exerpt Length", 'zr_core' ),
					"param_name" => "length",
					"admin_label" => true,
					"value" => 25,
					"description" => __( "Exerpt Length", 'zr_core' )
				 ),
				 array(
					"type" => "dropdown",
					"holder" => "div",
				...
				array(
			 	...
			 	array(
					...
					'value' => array(
						...
						__( 'Layout Style1','zr_core' ) => 'theme1',
					),
					'description' => sprintf( __( 'Select Layout', 'zr_core' ) )
				),	
				array(
					'type' => 'textfield',
					'heading' => __( 'Extra class name', 'zr_core' ),
					'param_name' => 'el_class',
					"admin_label" => true,
					'description' => __( 'If you wish to style particular content element differently, then use this field to add a class name and then refer to it in your css file.', 'zr_core' )
				),
			)
		) );
	}
	...
	**/
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('el_class'); ?>"><?php _e('El_class', 'zr_core')?></label>
			<br />
...
<p>
			<label for="<?php echo $this->get_field_id('style_title'); ?>"><?php _e('Style title ', 'zr_core')?></label>
			<br />
			<select class="widefat"
