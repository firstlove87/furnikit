...
class ZR_Widgets{
	...
	protected $fields = array(
			'zr_before_widget' => 'Before Widget',
			'zr_after_widget'  => 'After Widget',
			'zr_before_title'  => 'Before Title',
			'zr_after_title'   => 'After Title',
			'zr_display_conditions' => 'Display Conditions',
	...
	public function widget_display_callback( $instance, $widget, $args ){		
		...
		if ( zr_check_wdisplay($widget_display) == false ) return false;
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		$widget_advanced = zr_options('widget-advanced');
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'zr_core' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'zr_core' ) ?>
	...
	public function wp_enqueue_scripts(){
		if (!isset($this->_enqueue)){
			$this->initialize();
			wp_enqueue_style('widget-options', $this->url . 'admin/css/admin.css', array(), null);
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => esc_html__('All', 'zr_core'),
				'if_selected' => esc_html__('Only on selected', 'zr_core'),
				'if_no_selected' => esc_html__('Except selected', 'zr_core')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'zr_core' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => esc_html__('Show only for login users' , 'zr_core'),
				'logout' => esc_html__('Show only for logout users' , 'zr_core')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'zr_core' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => esc_html__('Blog posts', 'zr_core'),
					'archive' => esc_html__('Archive page', 'zr_core'),
					'date' => esc_html__('Archive Date', 'zr_core'),
					'category' => esc_html__('Category archive', 'zr_core'),
					'tag' => esc_html__('Tag archive', 'zr_core'),
					'author' => esc_html__('Author archive', 'zr_core'),
					'search' => esc_html__('Search page', 'zr_core'),
					'single' => esc_html__('Single post', 'zr_core'),
					'page' => esc_html__('Page template', 'zr_core'),
					'404' => esc_html__('Page 404', 'zr_core')
				);
			$html = '';
				...
				if(!isset($menu) || $menu != NULL || $menu != "")
					{
						...
						if (count($items) > 0) {
							$menu_checkbox = new ZR_Menu_Checkbox( $menu->slug);
...
$widgets = new ZR_Widgets;

/*
** Check widget display
*/
function zr_check_wdisplay ($widget_display){
	$widget_display = json_decode(json_encode($widget_display), true);
	$ZR_Menu_Checkbox = new ZR_Menu_Checkbox;
					...
					if ( ($key == 'login' && $user_ID) || ($key == 'logout' && !$user_ID) ){
						...
						if (isset($widget_display['checkbox']['menus'])) {
							...
							foreach ($widget_display['checkbox']['menus'] as $menu_id => $item_ids) {
								...
								if ( $ZR_Menu_Checkbox->is_menu_item_active($menu_id, $item_ids) ) return true;
							}
						}
					}
				}
		...
		if (isset($widget_display['checkbox'])) {
			...
			if (isset($widget_display['checkbox']['menus'])) {
				...
				foreach ($widget_display['checkbox']['menus'] as $menu_id => $item_ids) {
					...
					if ( $ZR_Menu_Checkbox->is_menu_item_active($menu_id, $item_ids) ) return false;
				}
			}			
		} else return false ;
	}
...
*/
class ZR_Menu_Checkbox extends Walker_Nav_Menu{
