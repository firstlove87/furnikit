...
if ( ! class_exists('ZR_Options') ){
	...
	class ZR_Options{
		public $dir = ZR_OPTIONS_DIR;
		public $url = ZR_OPTIONS_URL;
		public $page = '';
		public $args = array();
		...
		function _set_default_options(){
			...
			update_option( 'zr_google_fonts', $google_fonts );
		...
		function _enqueue(){
			...
			wp_enqueue_style(
				'zr-opts-css',
				ZR_OPTIONS_URL . '/options/css/options.css',
				array('farbtastic'),
				time(),
				'all'
			);
			...
			wp_enqueue_script(
				'zr-opts-js',
				ZR_OPTIONS_URL.'/options/js/options.js',
				array('jquery'),
				time(),
				true
			);
			
			wp_localize_script('zr-opts-js', 'zr_opts', array('reset_confirm' => esc_html__('Are you sure? Resetting will loose all custom values.', 'zr_core'), 'opt_name' => $this->args['opt_name']));
		...
		function _download_options(){
			...
			$backup_options['zr-opts-backup'] = '1';
		...
		function _load_page(){
			...
			do_action('zr-opts-load-page-'.$this->args['opt_name'], $screen);
				
		}//function


		/**
		 * do action zr-opts-admin-head for theme options page
		 *
		 * @since ZR_Options 1.0
		 */
		function admin_head(){
				
			do_action('zr-opts-admin-head-'.$this->args['opt_name'], $this);
		...
		function _register_setting(){
			...
			do_action('zr-opts-register-settings-'.$this->args['opt_name']);
		...
		function _validate_options($plugin_options){
			...
			set_transient('zr-opts-saved', '1', 1000 );
			...
			if(!empty($plugin_options['import'])){
				...
				if(is_array($imported_options) && isset($imported_options['zr-opts-backup']) && $imported_options['zr-opts-backup'] == '1'){
			...
			if($this->errors){
				set_transient('zr-opts-errors-'.$this->args['opt_name'], $this->errors, 1000 );
			}//if errors
				
			if($this->warnings){
				set_transient('zr-opts-warnings-'.$this->args['opt_name'], $this->warnings, 1000 );
			}//if errors
				
			do_action('zr-opts-options-validate-'.$this->args['opt_name'], $plugin_options, $this->options);
				...
				if(isset($section['fields'])){
					...
					foreach($section['fields'] as $fieldk => $field){
						...
						if(isset($field['validate'])){
							$validate = 'ZR_Validation_'.$field['validate'];
							
							if(!class_exists($validate)){
								include_once (ZR_OPTIONS_URL.'/options/validation/'.$field['validate'].'/validation_'.$field['validate'].'.php');
							}//if
		...
		public function _options_form(){
			...
			$this->options['last_tab'] = (isset($_GET['tab']) && !get_transient('zr-opts-saved'))?$_GET['tab']:$this->options['last_tab'];
			...
			<div class="cpannel-button">
				<button id="cpanel-submit" class="btn btn-inverse" type="submit"><?php esc_html_e( 'SAVE', 'zr_core' ); ?></button>
				<button id="cpanel-reset" class="btn btn-inverse" type="button"><?php esc_html_e( 'RESET', 'zr_core' ); ?></button>
			</div>
			<?php
			echo '</div>';
		...
		public function getFieldInstance( $field = array() ){
			...
			if ( !class_exists($classname) ){
				$classfile = ZR_OPTIONS_DIR."/options/fields/$type/field_$type.php";
		...
		public function getCpanelField( $field ){
			...
			$classname = 'ZR_Options_'.$field['type'];
			if ( !class_exists($classname) ){
				$classfile = ZR_OPTIONS_DIR.'/options/fields/'.$field['type'].'/field_'.$field['type'].'.php';
				if ( file_exists($classfile) ){
					include $classfile;
				}
			}
		...
		function _options_page_html(){
			...
			echo '<h2 id="zr-opts-heading">'.get_admin_page_title().'</h2>';
			
			echo isset($this->args['intro_text']) ? $this->args['intro_text'] : '';
		
			do_action('zr-opts-page-before-form-'.$this->args['opt_name']);
		
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="zr-options-form">';
			...
			echo '<div id="zr-opts-header">';
			submit_button('', 'primary', '', false);
			submit_button(esc_html__('Reset to Defaults', 'zr_core'), 'secondary', $this->args['opt_name'].'[defaults]', false);
			echo '<div class="clear"></div><!--clearfix-->';
			echo '</div>';
		
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('zr-opts-saved') == '1'){
				if(isset($this->options['imported']) && $this->options['imported'] == 1){
					echo '<div id="zr-opts-imported">'.apply_filters('zr-opts-imported-text-'.$this->args['opt_name'], wp_kses( __( '<strong>Settings Imported!</strong>', 'zr_core'), array( 'strong' => array() ) ) ).'</div>';
				}else{
					echo '<div id="zr-opts-save">'.apply_filters('zr-opts-saved-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings Saved!</strong>', 'zr_core'), array( 'strong' => array() ) ) ).'</div>';
					
				}
				delete_transient('zr-opts-saved');
			}
			echo '<div id="zr-opts-save-warn">'.apply_filters('zr-opts-changed-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings have changed!, you should save them!</strong>', 'zr_core'), array( 'strong' => array() ) ) ).'</div>';
			echo '<div id="zr-opts-field-errors">'. wp_kses(__('<strong><span></span> error(s) were found!</strong>', 'zr_core'), array( 'strong' => array(), 'span' => array() ) ).'</div>';
		
			echo '<div id="zr-opts-field-warnings">'.esc_html__('<strong><span></span> warning(s) were found!</strong>', 'zr_core').'</div>';
		
			echo '<div class="clear"></div><!--clearfix-->';
		
			echo '<div id="zr-opts-sidebar">';
			echo '<ul id="zr-opts-group-menu">';
			foreach($this->sections as $k => $section){
				$icon = (!isset($section['icon']))?'<img src="'.$this->url.'/options/img/glyphicons/glyphicons_019_cogwheel.png" alt=""/> ':'<img src="'.$section['icon'].'" alt=""/> ';
				echo '<li id="'.$k.'_section_group_li" class="zr-opts-group-tab-link-li">';
				echo '<a href="javascript:void(0);" id="'.$k.'_section_group_li_a" class="zr-opts-group-tab-link-a" data-rel="'.$k.'">'.$icon.'<span>'.$section['title'].'</span></a>';
				echo '</li>';
			}
				
			do_action('zr-opts-after-section-menu-items-'.$this->args['opt_name'], $this);
			
			echo '<li id="import_export_default_section_group_li" class="zr-opts-group-tab-link-li">';
			echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="zr-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'/options/img/glyphicons/glyphicons_082_roundabout.png" /> <span>'.esc_html__('Import / Export', 'zr_core').'</span></a>';
			echo '</li>';
		
		
			echo '<li id="plugins_default_section_group_li" class="zr-opts-group-tab-link-li">';
			echo '<a href="javascript:void(0);" id="plugins_default_section_group_li_a" class="zr-opts-group-tab-link-a custom-tab" data-rel="plugins_default"><img src="'.$this->url.'/options/img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.esc_html__('Advanced Plugins', 'zr_core').'</span></a>';
			echo '</li>';
		
			echo '</ul>';
			echo '</div>';
		
			echo '<div id="zr-opts-main">';
		
			foreach($this->sections as $k => $section){
				echo '<div id="'.$k.'_section_group'.'" class="zr-opts-group-tab">';
				do_settings_sections($k.'_section_group');
				echo '</div>';
			}
		
		
			echo '<div id="import_export_default_section_group'.'" class="zr-opts-group-tab">';
			echo '<h3>'.esc_html__('Import / Export Options', 'zr_core').'</h3>';

			echo '<h4>'.esc_html__('Import Options', 'zr_core').'</h4>';

			echo '<p><a href="javascript:void(0);" id="zr-opts-import-code-button" class="button-secondary">Import from file</a> <a href="javascript:void(0);" id="zr-opts-import-link-button" class="button-secondary">Import from URL</a></p>';

			echo '<div id="zr-opts-import-code-wrapper">';

			echo '<div class="zr-opts-section-desc">';

			echo '<p class="description" id="import-code-description">'.apply_filters('zr-opts-import-file-description',esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'zr_core')).'</p>';
			...
			echo '<div id="zr-opts-import-link-wrapper">';

			echo '<div class="zr-opts-section-desc">';

			echo '<p class="description" id="import-link-description">'.apply_filters('zr-opts-import-link-description',esc_html__('Input the URL to another sites options set and hit Import to load the options from that site.', 'zr_core')).'</p>';
			...
			echo '<p id="zr-opts-import-action"><input type="submit" id="zr-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.esc_attr__('Import', 'zr_core').'"> <span>'.apply_filters('zr-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'zr_core')).'</span></p>';
			echo '<div id="import_divide"></div>';

			echo '<h4>'.esc_html__('Export Options', 'zr_core').'</h4>';
			echo '<div class="zr-opts-section-desc">';
			echo '<p class="description">'.apply_filters('zr-opts-backup-description', esc_html__('Here you can copy/download your themes current option settings. Keep this safe as you can use it as a backup should anything go wrong. Or you can use it to restore your settings on this site (or any other site). You also have the handy option to copy the link to yours sites settings. Which you can then use to duplicate on another site', 'zr_core')).'</p>';
			echo '</div>';

			echo '<p><a href="javascript:void(0);" id="zr-opts-export-code-copy" class="button-secondary">Copy</a> <a href="'.add_query_arg(array('feed' => 'yaopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="zr-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="zr-opts-export-link" class="button-secondary">Copy Link</a></p>';
			$backup_options = $this->options;
			$backup_options['zr-opts-backup'] = '1';
			$encoded_options = '###'.serialize($backup_options).'###';
			echo '<textarea class="large-text" id="zr-opts-export-code" rows="8">';print_r($encoded_options);echo '</textarea>';
			echo '<input type="text" class="large-text" id="zr-opts-export-link-value" value="'.add_query_arg(array('feed' => 'yaopts-'.$this->args['opt_name'], 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" />';

			echo '</div>';		
		
		
				echo '<div id="plugins_default_section_group'.'" class="zr-opts-group-tab">';
				echo '<h2>'.esc_html__('Plugins On/Off', 'zr_core').'</h2>';
				echo '<table class="form-table"><tbody>';
				echo '<tr>
								<th scope="row">'. esc_html( 'Turn Off Testimonial', 'zr_core' ) .'</th>
							<td>
				...
				echo '<tr>
								<th scope="row">'. esc_html( 'Turn Off Our Team', 'zr_core' ) .'</th>
							<td>
								<table class="field-table">
									<tbody>
										<tr>
				...
				echo '<tr>
								<th scope="row">'. esc_html( 'Turn Off Portfolio', 'zr_core' ) .'</th>
							<td>
								<table class="field-table">
									<tbody>
										<tr>
			...
			do_action('zr-opts-after-section-items-'.$this->args['opt_name'], $this);
		
			echo '<div class="clear"></div><!--clearfix-->';
			echo '</div>';
			echo '<div class="clear"></div><!--clearfix-->';
		
			echo '<div id="zr-opts-footer">';
		
			if(isset($this->args['share_icons'])){
				echo '<div id="zr-opts-share">';
			...
			do_action('zr-opts-page-after-form-'.$this->args['opt_name']);
		...
		function _errors_js(){
			...
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('zr-opts-errors-'.$this->args['opt_name'])){
				$errors = get_transient('zr-opts-errors-'.$this->args['opt_name']);
				...
				echo 'jQuery("#zr-opts-field-errors span").html("'.count($errors).'");';
				echo 'jQuery("#zr-opts-field-errors").show();';
					
				foreach($section_errors as $sectionkey => $section_error){
					echo 'jQuery("#'.$sectionkey.'_section_group_li_a").append("<span class=\"zr-opts-menu-error\">'.$section_error.'</span>");';
				}
					
				foreach($errors as $error){
					echo 'jQuery("#'.$error['id'].'").addClass("zr-opts-field-error");';
					echo 'jQuery("#'.$error['id'].'").closest("td").append("<span class=\"zr-opts-th-error\">'.$error['msg'].'</span>");';
				}
				echo '});';
				echo '</script>';
				delete_transient('zr-opts-errors-'.$this->args['opt_name']);
			}
		 ...
		 *
		 * @since ZR_Options 1.0.3
		 */
		function _warnings_js(){
				
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('zr-opts-warnings-'.$this->args['opt_name'])){
				$warnings = get_transient('zr-opts-warnings-'.$this->args['opt_name']);
				...
				echo '<script type="text/javascript">';
				echo 'jQuery(document).ready(function(){';
				echo 'jQuery("#zr-opts-field-warnings span").html("'.count($warnings).'");';
				echo 'jQuery("#zr-opts-field-warnings").show();';
					
				foreach($section_warnings as $sectionkey => $section_warning){
					echo 'jQuery("#'.$sectionkey.'_section_group_li_a").append("<span class=\"zr-opts-menu-warning\">'.$section_warning.'</span>");';
				}
					
				foreach($warnings as $warning){
					echo 'jQuery("#'.$warning['id'].'").addClass("zr-opts-field-warning");';
					echo 'jQuery("#'.$warning['id'].'").closest("td").append("<span class=\"zr-opts-th-warning\">'.$warning['msg'].'</span>");';
				}
				echo '});';
				echo '</script>';
				delete_transient('zr-opts-warnings-'.$this->args['opt_name']);
			}
		...
		function _section_desc($section){
			...
			echo '<table class="zr-opts-section-desc"><tr><td>';
