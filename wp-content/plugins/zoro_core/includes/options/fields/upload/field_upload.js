(function($) {
	$(document).ready(function(){
		...
		$("img[src='']").attr("src", zr_upload.url);
		
		$('.zr-menu-upload').on('click', function() {
			header_clicked = true;
		...
		$('.zr-opts-upload-remove').click(function(){
			$relid = $(this).attr('rel-id');
			$('#'+$relid).val('');
			$(this).prev().fadeIn('fast');
			$(this).prev().prev().fadeOut('fast', function(){jQuery(this).attr("src", zr_upload.url);});
			$(this).fadeOut('slow');
		});
		
		$('.zr-menu-upload-remove').on('click', function(){
			$relid = $(this).attr('rel-id');
			$('#'+$relid).val('');
			$(this).prev().fadeIn('fast');
			$(this).prev().prev().fadeOut('fast', function(){$(this).attr("src", zr_menu_upload.url);});
			$(this).fadeOut('slow');
		});
	});
	function MenuClick(){
