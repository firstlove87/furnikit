...
if ( !class_exists( 'Radium_Theme_Demo_Data_Importer' ) ) {	
	...
	class Radium_Theme_Demo_Data_Importer extends Radium_Theme_Importer {
		 ...
		 * @var string
		 */
		public $theme_option_name       = 'revo_theme'; //set theme options name here (key used to save theme options). Optiontree option name will be set automatically
		...
		public function __construct() { 
			...
			$sw_actionvation = get_option( 'sw_activation' );
			
			$this->content_demo_file_name = 'data.xml';
			if( $sw_actionvation ){
				$this->content_demo_file_name = 'data-page.xml';
			}

			$this->demo_files_path = dirname(__FILE__) . '/demo-data/'; 
		...
		public function set_demo_menus(){
			if( !get_option( 'sw_activation' ) ){
				// Menus to Import and assign - you can remove or add as many as you want
				$top_menu    = get_term_by('name', 'Primary Menu', 'nav_menu');  
				$left_menu   = get_term_by('name', 'Verticle Menu', 'nav_menu');
				$menu_mobile = get_term_by('name', 'Menu Mobile 1', 'nav_menu');
				$menu_mobile1 = get_term_by('name', 'Menu Mobile 2', 'nav_menu');
				$menu_mRTL   = get_term_by('name', 'Mobile Menu RLT', 'nav_menu');

				set_theme_mod( 'nav_menu_locations', array(
						'primary_menu' => $top_menu->term_id,
						'vertical_menu' => $left_menu->term_id,	
						'mobile_menu1' => $menu_mobile->term_id,	
						'mobile_menu2' => $menu_mobile1->term_id,	
						'mobile_menu_rtl' => $menu_mRTL->term_id,	
					)
				);
