...
class Furnikit_Resmenu{
	...
	function Furnikit_MenuRes_AdFilter( $args ){
		...
		foreach( $menu as $furnikit_theme_locate => $menu_description ){
			if( $furnikit_theme_locate == $args['theme_location'] ){
				...
				$args['menu_class'].= ($args['menu_class'] == '' ? '' : ' ') . 'furnikit-menures';			
	...
	function ResNavMenu( $args ){
		$args['furnikit_resmenu'] = true;		
		$select = wp_nav_menu( $args );
		return $select;
	}
	function Furnikit_MenuRes_Filter( $args ){
		/* Fix Menu on wp 4.7 */
		...
		foreach( $menu as $furnikit_theme_locate => $menu_description ){
			if( $furnikit_theme_locate == $args['theme_location'] ){
				...
				$args['items_wrap']	= '<button class="navbar-toggle" type="button" data-toggle="collapse" data-target="#ResMenu'. esc_attr( $furnikit_theme_locate ) .'">
					<span class="sr-only">Categories</span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
				</button><div id="ResMenu'. esc_attr( $furnikit_theme_locate ) .'" class="collapse menu-responsive-wrapper"><ul id="%1$s" class="%2$s">%3$s</ul></div>';	
