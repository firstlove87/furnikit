...
if ( ! class_exists( 'WC_Currency_Converter' ) ) {
	...
	class WC_Currency_Converter {
		...
		public function __construct() {
			$this->settings = array(
				array(
					'name' => esc_html__( 'Open Exchange Rate API', 'furnikit' ),
					'type' => 'title',
					'desc' => '',
					'id'   => 'woocommerce-currency-converter-widget'
				),
				array(
					'name' => esc_html__( 'App Key', 'furnikit' ),
					'desc' => sprintf( __( '(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'furnikit' ), 'https://openexchangerates.org/signup' ),
		...
		public function admin_notice_wrong_key() {
			?>
			<div class="error">
				<p><?php esc_html_e( 'WooCommerce Currency Converter: Incorrect key entered!', 'furnikit' ); ?></p>
			</div>
		<?php
		}
		...
		public function load_plugin_textdomain() {
			load_plugin_textdomain( 'furnikit', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
		...
		public function get_converter_form( $instance, $echo = true ) {
			...
			$html = '<form method="post" id="currency_converter" action="'. esc_url( home_url('/') ) .'">' . "\n";
			...
			if ( $currencies ) {
				...
				if ( ! empty( $instance['currency_display'] ) && 'select' === $instance['currency_display'] ) {
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= ' <a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'furnikit' ) . '</a>';
					}

				} else {
					$html .= '<ul class="currency_w">';
					$html .= '<li><a href="#" class="" >' . $currencies[0] . '</a>';
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= '<li><a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'furnikit' ) . '</a></li>';
					}
			...
			$html .= "<script>jQuery(document).ready(function($) { wc_currency_converter_params.current_currency = '" . esc_js( $current_currency ) . "'; });</script>";


			$html .= "<script>jQuery(document).ready(function($) { wc_currency_converter_params.symbol_positions = " . json_encode( $symbol_positions ) . "; });</script>";
		...
		public function widgets() {
			include_once( get_template_directory(). '/lib/plugins/currency-converter/currency-converter-widget.php' );
		}
		...
		public function enqueue_assets() {
			...
			// Scripts
			wp_register_script( 'moneyjs', get_template_directory_uri() .'/lib/plugins/currency-converter/assets/js/money.min.js', array( 'jquery' ), null, true );
			wp_register_script( 'accountingjs', get_template_directory_uri() .'/lib/plugins/currency-converter/assets/js/accounting.min.js', array( 'jquery' ), null, true );
			wp_register_script( 'wc_currency_converter', get_template_directory_uri() .'/lib/plugins/currency-converter/assets/js/conversion.js', array(
				'jquery',
				'moneyjs',
				'accountingjs',
			), null, true );
			...
			$wc_currency_converter_params = array(
				...
				'i18n_oprice'            => esc_html__( 'Original price:', 'furnikit' ),
