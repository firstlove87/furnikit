...
*/
add_action( 'wcvendors_before_dashboard', 'furnikit_wrapper_before_vendor_dashboard' );
add_action( 'wcvendors_after_dashboard', 'furnikit_wrapper_after_vendor_dashboard' );
function furnikit_wrapper_before_vendor_dashboard(){
	echo '<div class="vendor-dashboard-wrapper">';
}

function furnikit_wrapper_after_vendor_dashboard(){
	echo '</div>';
}

add_action( 'wp', 'furnikit_wcvendor_hook' );
function furnikit_wcvendor_hook(){
	...
	if( 'pro' !== $pro_store_header ) {
		...
		if( WCV_Vendors::is_vendor_page() ) {
			add_action( 'woocommerce_before_main_content', 'furnikit_vendor_breadcrumb', 9 );
...
function furnikit_vendor_breadcrumb(){
?>
...
** Add wrapper sold by
*/
add_filter( 'wcvendors_sold_by_in_loop', 'furnikit_custom_soldby' );
function furnikit_custom_soldby( $sold_by_label ){
