...
if ( !class_exists('Furnikit_Widget') ):

abstract class Furnikit_Widget extends WP_Widget{
	...
	protected function init(){
		$this->base_path = get_template_directory().'/lib';
		$this->base_tpl_path = apply_filters('furnikit_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
		$this->override_tpl_path = apply_filters('furnikit_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
	}
	...
	public function furnikit_trim_words( $text, $num_words = 30, $more = null ) {
		...
		if ( is_null($this->tpls) ){
			...
			if ( !$has_default ){
				...
				try{
					...
					wp_mkdir_p( dirname( $default_tpl ) );
					global $wp_filesystem;
	...
	public function widget_template_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( array_key_exists('ov', $templates) && count($templates['ov']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Override by Theme', 'furnikit') .'">';
		...
		if ( array_key_exists('df', $templates) && count($templates['df']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Default Template', 'furnikit') .'">';
	...
	public function category_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( (is_string($opts['multiple']) && strtolower($opts['multiple'])=='multiple') || (is_bool($opts['multiple']) && $opts['multiple']) ){
			...
			if (array_key_exists('allow_select_all', $opts) && $opts['allow_select_all']){
				...
				$allow_select_all = '<option value="0">'. esc_html_e( 'All Categories', 'furnikit' ) .'</option>';
			}
		}
		...
		foreach ($categories as $cat){
			$select_html .= '<option value="' . esc_attr( $cat->cat_ID ) . '"';
	...
	public function product_select( $field_name, $opts = array(), $field_value = null ){
		...
		if (isset($allow_select_all)) $select_html .= $allow_select_all;
		foreach ($categories as $cat){
			$select_html .= '<option value="' . esc_attr( $cat->term_id ) . '"';
	...
	public function form( $instance ){
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('widget_template') ); ?>"><?php esc_html_e("Template", 'furnikit') ?></label>
			<br/>
