 ...
 */
function furnikit_unregister_default_widgets() {
...
add_action('widgets_init', 'furnikit_unregister_default_widgets', 11);
 
/*
** Register Sidebar and Widgets
*/
function furnikit_widgets_init() {
	
	// Sidebars
	global $furnikit_widget_areas;
	$furnikit_widget_areas = furnikit_widget_setup_args();
	if ( count($furnikit_widget_areas) ){
		foreach( $furnikit_widget_areas as $sidebar ){
			$sidebar_params = apply_filters('furnikit_sidebar_params', $sidebar);
			register_sidebar($sidebar_params);
		}
	}

	// Widgets
	register_widget('Furnikit_Social_Widget');
	register_widget('Furnikit_Top_Widget');
}
add_action('widgets_init', 'furnikit_widgets_init');
...
class Furnikit_Top_Widget extends Furnikit_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'zr_top', 'description' => esc_html__('SW top header widget', 'furnikit'));
		parent::__construct('zr_top', esc_html__('SW Top Widget', 'furnikit'), $widget_ops);
	}
}

class Furnikit_Social_Widget extends WP_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'zr_social', 'description' => esc_html__('SW Social Networks', 'furnikit'));
		parent::__construct('zr_social', esc_html__('SW Social', 'furnikit'), $widget_ops);
	...
	function form($instance){ 
		...
		?>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title: ', 'furnikit'); ?>
...
<?php 
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo esc_html__( 'Social', 'furnikit' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'furnikit')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'furnikit')?> </label>
...
<?php
		...
		// furnikit fields for add new social network
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'furnikit' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'furnikit'); ?>
	</label> <input class="widefat" id="<?php echo esc_attr( $id_i_icon ); ?>"
		name="<?php echo esc_attr( $name_i_icon ); ?>" type="text" value=""
		placeholder="<?php esc_attr_e( 'Enter Font Awesome icon or Title', 'furnikit' ) ?>" /> <span><?php esc_html_e( 'If using as
		icon, please choose class name in Font Awesome. This is required.', 'furnikit' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'furnikit')?> </label>
