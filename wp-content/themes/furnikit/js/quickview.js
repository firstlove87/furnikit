	...
	**/
	if( $('body').html().match( /zr-quickview-bottom/ ) ){
		...
		$(document).on( 'click', 'a[data-type="quickview"]', function(){
			var product_id = $(this).data( 'product_id' ), ajaxurl = quickview_param.ajax_url.replace( '%%endpoint%%', 'secretsho_quickviewproduct' );
			ajaxurl = ajaxurl.replace( '%endpoint%', 'secretsho_quickviewproduct' );
			qv_target.addClass( 'show loading' );
			var data 		= {
				action: 'secretsho_quickviewproduct',
